...
// Generate generates mock structs based on the given configuration.
func Generate(c Config) {
	...
	outputPackageName := c.PackageOut
	...
	outputPackagePath := c.SelfPackage
	if outputPackagePath == "" && c.Destination != "" {
		dstPath, err := filepath.Abs(filepath.Dir(c.Destination))
		if err == nil {
	...
	if c.MockNames != "" {
		g.mockNames = parseMockNames(c.MockNames)
	}
	if c.CopyrightFile != "" {
		header, err := os.ReadFile(c.CopyrightFile)
		if err != nil {
	...
	if len(c.Destination) > 0 {
		if err := os.MkdirAll(filepath.Dir(c.Destination), os.ModePerm); err != nil {
			log.Fatalf("Unable to create directory: %v", err)
		}
		existing, err := os.ReadFile(c.Destination)
...
func (g *generator) Generate(pkg *model.Package, outputPkgName string, outputPackagePath string) error {
	if outputPkgName != pkg.Name && g.c.SelfPackage == "" {
	...
	if g.c.Imports != "" {
		for _, kv := range strings.Split(g.c.Imports, ",") {
			eq := strings.Index(kv, "=")
...
func (g *generator) GenerateMockInterface(intf *model.Interface, outputPackagePath string) error {
	...
	g.GenerateMockMethods(mockType, intf, outputPackagePath, longTp, shortTp, g.c.Typed)
